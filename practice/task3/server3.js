...
*/
app.get('/fit', (req, res) => {
	let search = req.query.length;
	let results = [];
	console.log(`Finding campgrounds that can support a ${search} foot RV`);
	for(const camp of campgrounds){
	    if(camp.lengthLimit >= search){
		    results.push({campground: camp.name, location : camp.town, maxLength: camp.lengthLimit});
		}
	}
...
app.get('/elevation', (req,res) => {
	let sites = [];
	let el = req.query.altitude;
	let dir = req.query.direction;
	console.log(`Getting campgrounds ${dir} than ${el} feet`);
	if(dir == "higher"){
		for(const camp of campgrounds){
			if(camp.elevation >= el){
				sites.push({campground: camp.name, elevation: camp.elevation, town: camp.town});
			}
		}
	}else if(dir == "lower"){
		for(const camp of campgrounds){	
